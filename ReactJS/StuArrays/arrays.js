...
let names = princesses.map(p => p.name);
console.log(`Names --> ${names}`);
...
let mNames = names.filter(data => data.startsWith("M"));
...
let lNames = princesses.filter(data => data.name.includes("l"));
const lsum = lNames.map(data => data.age).reduce((lsum, lnameAge) => lsum + lnameAge);
console.log("Average of L names --> " + lsum/lNames.length);
